...
const Main = ({ user }) => {
  ...
  // Filtreleme yarları tanımla
  const options = query(tweetsCol, orderBy('createdAt','desc'))

  

  useEffect(() => {
    // Tweetler koleksiyonuna abone olman gerekiyor
    const unsub = onSnapshot(options, (snapshot) => {
      // geçici dizi
      const tempTweets = []

      // Tüm dökümanları dolaşıp her birinin veri ve id’sini içeren objeleri geçici diziye ekle
